import mjml2html from "mjml";
import nodemailer from "nodemailer";

// Create reusable transporter with updated env vars
const transporter = nodemailer.createTransport({
  host: process.env.SMTP_SERVER_HOST,
  port: Number(process.env.SMTP_SERVER_PORT) || 587,
  secure: process.env.SMTP_SERVER_SECURE === "true",
  auth: {
    user: process.env.SMTP_SERVER_USER,
    pass: process.env.SMTP_SERVER_PASSWORD,
  },
});

interface SendVerificationEmailParams {
  email: string;
  firstName: string;
  verificationUrl: string;
}

export async function sendVerificationEmail({
  email,
  firstName,
  verificationUrl,
}: SendVerificationEmailParams) {
  const mjmlTemplate = `
    <mjml>
      <mj-head>
        <mj-title>Confirmez votre compte - Collection Wars</mj-title>
        <mj-preview>Bienvenue sur Collection Wars ! Confirmez votre adresse email pour commencer.</mj-preview>
        <mj-attributes>
          <mj-all font-family="Arial, sans-serif" />
          <mj-text font-size="14px" color="#555555" line-height="20px" />
          <mj-section background-color="#ffffff" padding="20px" />
        </mj-attributes>
        <mj-style inline="inline">
          .button-link { text-decoration: none; }
        </mj-style>
      </mj-head>
      <mj-body background-color="#f4f4f4">
        <!-- Header -->
        <mj-section background-color="#1a1a2e" padding="30px 20px">
          <mj-column>
            <mj-text align="center" color="#ffffff" font-size="28px" font-weight="bold">
              Collection Wars
            </mj-text>
          </mj-column>
        </mj-section>

        <!-- Main Content -->
        <mj-section background-color="#ffffff" padding="40px 30px">
          <mj-column>
            <mj-text font-size="20px" color="#333333" font-weight="bold" padding-bottom="20px">
              Bonjour ${firstName} !
            </mj-text>

            <mj-text color="#555555" padding-bottom="10px">
              Bienvenue sur <strong>Collection Wars</strong> ! Nous sommes ravis de vous compter parmi nous.
            </mj-text>

            <mj-text color="#555555" padding-bottom="20px">
              Pour finaliser la création de votre compte et commencer à jouer, veuillez confirmer votre adresse email en cliquant sur le bouton ci-dessous :
            </mj-text>

            <mj-button
              background-color="#7c3aed"
              color="#ffffff"
              font-size="16px"
              font-weight="bold"
              href="${verificationUrl}"
              padding="20px 0"
              border-radius="8px"
              css-class="button-link"
            >
              Confirmer mon email
            </mj-button>

            <mj-text color="#888888" font-size="12px" padding-top="20px">
              Si le bouton ne fonctionne pas, copiez et collez ce lien dans votre navigateur :
            </mj-text>

            <mj-text color="#7c3aed" font-size="12px" padding-top="5px">
              ${verificationUrl}
            </mj-text>

            <mj-divider border-color="#e0e0e0" padding="30px 0" />

            <mj-text color="#888888" font-size="12px">
              Ce lien de vérification expirera dans 24 heures. Si vous n'avez pas créé de compte sur Collection Wars, vous pouvez ignorer cet email en toute sécurité.
            </mj-text>
          </mj-column>
        </mj-section>

        <!-- Footer -->
        <mj-section background-color="#f4f4f4" padding="20px">
          <mj-column>
            <mj-text align="center" color="#888888" font-size="12px">
              © ${new Date().getFullYear()} Collection Wars. Tous droits réservés.
            </mj-text>
            <mj-text align="center" color="#888888" font-size="12px" padding-top="10px">
              Cet email a été envoyé à ${email}
            </mj-text>
          </mj-column>
        </mj-section>
      </mj-body>
    </mjml>
  `;

  const { html, errors } = mjml2html(mjmlTemplate);

  if (errors.length > 0) {
    console.error("MJML compilation errors:", errors);
    throw new Error("Failed to compile email template");
  }

  try {
    await transporter.sendMail({
      from: `"Collection Wars" <${
        process.env.SMTP_FROM || process.env.SMTP_USER
      }>`,
      to: email,
      subject: "Confirmez votre compte - Collection Wars",
      html,
    });

    console.log(`Verification email sent to ${email}`);
  } catch (error) {
    console.error("Error sending verification email:", error);
    throw new Error("Failed to send verification email");
  }
}

interface SendMagicLinkParams {
  email: string;
  url: string;
}

/**
 * Envoie un email avec lien magique de connexion
 * Utilisé par Next-auth EmailProvider
 */
export async function sendMagicLinkEmail({ email, url }: SendMagicLinkParams) {
  const mjmlTemplate = `
    <mjml>
      <mj-head>
        <mj-title>Connexion à Collection Wars</mj-title>
        <mj-preview>Cliquez sur le lien pour vous connecter à votre compte</mj-preview>
        <mj-attributes>
          <mj-all font-family="Arial, sans-serif" />
          <mj-text font-size="14px" color="#555555" line-height="20px" />
          <mj-section background-color="#ffffff" padding="20px" />
        </mj-attributes>
      </mj-head>
      <mj-body background-color="#f4f4f4">
        <!-- Header -->
        <mj-section background-color="#1a1a2e" padding="30px 20px">
          <mj-column>
            <mj-text align="center" color="#ffffff" font-size="28px" font-weight="bold">
              Collection Wars
            </mj-text>
          </mj-column>
        </mj-section>

        <!-- Main Content -->
        <mj-section background-color="#ffffff" padding="40px 30px">
          <mj-column>
            <mj-text font-size="20px" color="#333333" font-weight="bold" padding-bottom="20px">
              Connexion à votre compte
            </mj-text>

            <mj-text color="#555555" padding-bottom="10px">
              Vous avez demandé à vous connecter à <strong>Collection Wars</strong>.
            </mj-text>

            <mj-text color="#555555" padding-bottom="20px">
              Cliquez sur le bouton ci-dessous pour vous connecter en toute sécurité :
            </mj-text>

            <mj-button
              background-color="#7c3aed"
              color="#ffffff"
              font-size="16px"
              font-weight="bold"
              href="${url}"
              padding="20px 0"
              border-radius="8px"
            >
              Se connecter
            </mj-button>

            <mj-text color="#888888" font-size="12px" padding-top="20px">
              Si le bouton ne fonctionne pas, copiez et collez ce lien dans votre navigateur :
            </mj-text>

            <mj-text color="#7c3aed" font-size="12px" padding-top="5px">
              ${url}
            </mj-text>

            <mj-divider border-color="#e0e0e0" padding="30px 0" />

            <mj-text color="#888888" font-size="12px">
              Ce lien de connexion expirera dans 24 heures. Si vous n'avez pas demandé cette connexion, vous pouvez ignorer cet email en toute sécurité.
            </mj-text>
          </mj-column>
        </mj-section>

        <!-- Footer -->
        <mj-section background-color="#f4f4f4" padding="20px">
          <mj-column>
            <mj-text align="center" color="#888888" font-size="12px">
              © ${new Date().getFullYear()} Collection Wars. Tous droits réservés.
            </mj-text>
            <mj-text align="center" color="#888888" font-size="12px" padding-top="10px">
              Cet email a été envoyé à ${email}
            </mj-text>
          </mj-column>
        </mj-section>
      </mj-body>
    </mjml>
  `;

  const { html, errors } = mjml2html(mjmlTemplate);

  if (errors.length > 0) {
    console.error("MJML compilation errors:", errors);
    for (const err of errors) {
      console.error(err.formattedMessage);
    }
    throw new Error("Failed to compile email template");
  }

  try {
    await transporter.sendMail({
      from: process.env.SMTP_FROM || "noreply@collectionwars.com",
      to: email,
      subject: "Connexion à Collection Wars",
      html,
    });

    console.log(`Magic link email sent to ${email}`);
  } catch (error) {
    console.error("Error sending magic link email:", error);
    throw new Error("Failed to send magic link email");
  }
}

// Test email connection
export async function testEmailConnection() {
  try {
    await transporter.verify();
    console.log("✅ SMTP connection successful");
    return true;
  } catch (error) {
    console.error("❌ SMTP connection failed:", error);
    return false;
  }
}
